...
export default function EmployeeProfileTab({ employeeId, onBack }: { employeeId: number; onBack: () => void }) {
  ...
  const stats = [
    { l: 'إجمالي الحضور', v: balanceData.totalPresent, bg: 'bg-blue-50 border-blue-100', tc: 'text-blue-700' },
    { l: 'إجازات مستحقة', v: balanceData.earned, bg: 'bg-green-50 border-green-100', tc: 'text-green-700' },
    { l: 'إجازات مأخوذة', v: balanceData.taken, bg: 'bg-orange-50 border-orange-100', tc: 'text-orange-700' },
    { l: 'صافي الرصيد المتاح', v: finalBalance, bg: finalBalance < 0 ? 'bg-red-50 border-red-100' : 'bg-emerald-50 border-emerald-100', tc: finalBalance < 0 ? 'text-red-700' : 'text-emerald-700' },
