...
import {
  ...
  printMachineryOwnerSheet,
} from '../lib/printMachineryWeekly';
...
export default function MachineryTab({ user }: Props) {
  ...
  // قائمة الأسابيع المنسقة (من السبت إلى الجمعة) للاختيار السريع
  ...
  const recentWeeksList = useMemo(() => {
    ...
    for (let i = 0; i < 24; i++) {
      ...
      list.push({
        sat: satStr,
        fri: friStr,
        label: `${satStr} (السبت) ⬅ إلى ➡ ${friStr} (الجمعة)${isCurrent ? ' 🌟 [الأسبوع الحالي]' : ''}`,
      });
    }
    return list;
  }, [currentSaturday]);
      ...
      <div className="rounded-[2rem] border border-slate-200 bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 p-5 text-white shadow-sm flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs font-bold text-slate-300 mt-0.5">تسجيل يومي · تقارير أسبوعية (من السبت إلى الجمعة) وشهرية شاملة · كشوفات تفصيلية وإكسيل</p>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[400] overflow-y-auto bg-slate-950/70 p-4" onClick={() => setPrintOwner(null)}>
          <div className="mx-auto max-w-2xl" onClick={e => e.stopPropagation()}>
            <div className="mb-3 flex gap-2 print:hidden">
              ...
              <button type="button" onClick={() => setPrintOwner(null)} className="rounded-xl bg-white px-5 py-3 text-sm font-black text-slate-700 hover:bg-slate-100 cursor-pointer">إغلاق</button>
            </div>
            {(() => {
              const o = ownersList.find(x => x.owner === printOwner);
              if (!o) return null;
