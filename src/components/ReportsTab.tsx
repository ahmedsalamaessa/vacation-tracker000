...
export default function ReportsTab({ user }: Props) {
  ...
  function buildReportRows() {
    return filteredEmployees.map(emp => {
      const counts = summary.get(emp.id) ?? {};
  ...
  function doExport() {
    ...
    exportToCSV(rows, `تقرير_${year}_${pad(month + 1)}${selectedLocationName ? `_${selectedLocationName}` : ''}${searchSuffix}`, headers);
  }

  function doPdfExport() {
    const rows = buildReportRows();
    const headerCells = ['الموظف', 'الوظيفة', ...COLUMNS.map(c => c.label)].map(h => `<th>${h}</th>`).join('');
    const bodyRows = rows.map(row => { const cells = [row.name, row.jobTitle, ...COLUMNS.map(c => row[c.key] ?? 0)].map(v => `<td>${String(v ?? '')}</td>`).join(''); return `<tr>${cells}</tr>`; }).join('');
    const selectedLocationName = locations.find(loc => loc.id === Number(locationId))?.name;
    const searchTitle = employeeSearch.trim() ? ` - ${employeeSearch.trim()}` : '';
    printHtml(`تقرير حضور شهري ${year}-${pad(month + 1)}${selectedLocationName ? ` - ${selectedLocationName}` : ''}${searchTitle}`, `<table><thead><tr>${headerCells}</tr></thead><tbody>${bodyRows}</tbody></table>`);
  }
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {!isEmployee && (
            <select value={locationId} onChange={(e) => setLocationId(e.target.value)} className="border border-slate-300 rounded-lg px-3 py-2 text-sm">
              ...
              {locations.filter(l => user.role === 'admin' || user.locationIds.includes(l.id)).map(loc => (<option key={loc.id} value={loc.id}>{loc.name}</option>))}
            </select>
          )}
      ...
      {loading ? (<div className="text-center text-slate-500 py-8">جاري التحميل...</div>) : filteredEmployees.length === 0 ? (<div className="text-center text-slate-500 py-8">لا توجد بيانات</div>) : (
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-center border-collapse">
            <thead><tr className="bg-slate-100"><th className="p-2 border border-slate-200 text-right min-w-[140px]">الموظف</th>{COLUMNS.map(c => (<th key={c.key} className="p-2 border border-slate-200"><div>{c.emoji}</div><div className="text-[10px] text-slate-500">{c.label}</div></th>))}</tr></thead>
            <tbody>
              {filteredEmployees.map(emp => {
                const counts = summary.get(emp.id) ?? {};
                return (<tr key={emp.id}><td className="p-2 border border-slate-200 text-right"><div className="font-medium">{emp.name}</div><div className="text-[10px] text-slate-400">{emp.jobTitle || '—'}</div></td>{COLUMNS.map(c => (<td key={c.key} className={`p-2 border border-slate-200 font-bold ${c.color}`}>{counts[c.key] || 0}</td>))}</tr>);
              })}
            </tbody>
          </table>
        </div>
